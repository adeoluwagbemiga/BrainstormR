...
{
    public class AskAnExpertService : IAskAnExpertService
    {
        public async Task<IEnumerable<InstructingExpertItemModel>> getAllExpert(string accessToken)
        {
            try
            {
                ...
                using (var client = new HttpClient())
                {
                    ...
                    var response = await client.GetAsync("/api/Remote/Experts");
        ...
        public async Task<InstructingExpertItemModel> getExpert(string accessToken, int instructorId)
        {
            try
            {
                InstructingExpertItemModel result = null;
                var instructorlist = await getAllExpert(accessToken);
                result = instructorlist.FirstOrDefault(a => a.Id == instructorId && a.IsExpert == true && a.IsTutor == true);
                return result;
            }
            catch (Exception ex)
            {
